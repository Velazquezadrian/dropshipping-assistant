...
_{timestamp}_""",
        ...
        templates['high_quality'] = NotificationTemplate(
            ...
            body_template="""✨ **PRODUCTO PREMIUM** ✨
...
🔗 {url}""",
    ...
    def _prepare_variables(self, product: Product, priority: NotificationPriority) -> Dict[str, Any]:
        ...
        # Crear descripción basada en los datos disponibles
        description_parts = []
        if product.category:
            description_parts.append(f"Categoría: {product.category}")
        if product.source_platform:
            description_parts.append(f"Fuente: {product.source_platform}")
        if product.shipping_time:
            description_parts.append(f"Envío en {product.shipping_time} días")
        
        description = " | ".join(description_parts) if description_parts else "Producto de calidad disponible"
    ...
    def _create_discord_embed(self, 
        ...
        if platform_config.get('thumbnail') and hasattr(product, 'image_url') and product.image_url:
    ...
    def _create_fallback_notification(self, 
        ...
        simple_message = f"""🆕 Nuevo Producto
